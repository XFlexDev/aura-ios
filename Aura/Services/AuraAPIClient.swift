...
@Observable final class AuraAPIClient {
    ...
    func files(in bucket: B2Bucket, prefix: String? = nil) async throws -> [B2File] { var path = "/buckets/\(bucket.id)/files"; if let prefix { path += "?prefix=\(prefix.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? prefix)" }; return try await get(path) }
    func status() async throws -> VPSStatus { try await get("/status") }
    private func get<T: Decodable>(_ path: String) async throws -> T { var request = URLRequest(url: baseURL.appendingPathComponent(path)); request.setValue("application/json", forHTTPHeaderField: "Accept"); if let token { request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }; let (data, response) = try await session.data(for: request); guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }; let decoder = JSONDecoder(); decoder.dateDecodingStrategy = .iso8601; return try decoder.decode(T.self, from: data) }
}
